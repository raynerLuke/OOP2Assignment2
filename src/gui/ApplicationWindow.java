...
public class ApplicationWindow extends Application {
	...
	private ListView<Patient> lview;
	private ObservableList<Patient> observableView;
	private ComboBox<String> genderComboBox;
	private Button closeButton;
	
	public void start(Stage stage) throws Exception 
	{
		stage.setTitle("Doctors Surgery Appllication");
	...
	private Node topPane()
	{
	    ...
	    Text title = new Text("Doctor's Surgery");
	...
	private Node leftPane()
	{
		...
		lview = new ListView<Patient>();
		pane.add(lview, 0, 0);
		pane.setAlignment(Pos.TOP_RIGHT);
		pane.setPadding(new Insets(20,20,10,45));
		pane.setStyle("-fx-background-color : #8888AA");
        
		ArrayList<Patient> patients = PatientController.getInstance().getAllPatients();
		this.observableView = FXCollections.observableList(patients);
	...
	private Node rightPane()
	{
	    ...
	    loadButton.setOnAction(new EventHandler<ActionEvent>(){
	    	public void handle(ActionEvent e)
	    	{
	    	    PatientController.getInstance().load();
	    	    ArrayList<Patient> loadedList = PatientController.getInstance().getAllPatients();
